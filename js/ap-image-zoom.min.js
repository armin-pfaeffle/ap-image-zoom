...
;(function($) {
	...
	var PLUGIN_DATE = "04.09.2014";


	/**
	 *
	 */
	function ApImageZoom(target, options) {
		this.$target = $(target);
	 ...
	 *
	 */
	Apiz.prototype._init = function() {
		var self = this;

		// Backup current style so we can restore it after destruction
		this.originalStyle = this.attr('style'); // TODO: in destroy einbauen

		this.isPanning = false;
		this.isPinching = false;

		// Make a temporary hidden copy of image, so we obtain the real size of the image
		var imageCopy = $('<img />').hide().appendTo('body');
		imageCopy.load(function() {
			self._obtainImageProperties(imageCopy);
			self._setup();
			// obtainImageProperties(imageCopy);
			// initialize();
			$(this).remove();
		});
		imageCopy.attr('src', this.attr('src'));
	};
	...
	$.fn.apImageZoom = function( action, options )
	{
		...
		function create($image)
		{
			...
			this.targetImage.data('panning', false);
			this.targetImage.data('pinching', false);
		...
		{
			var settings = this.targetImage.data('settings');
			if (!settings.disableHammerPlugin && typeof Hammer == "function") {
				setupHammer();
			}
			if (!settings.disableMouseWheelPlugin && typeof jQuery.fn.mousewheel == "function") {
		...
		function onPan(evt) {
			...
			if (image.data('settings').disabled || image.data('settings').disablePan) {
				return;
			}

			var currentPosition = new Point(image.css('left'), image.css('top'));
			if (evt.type == 'panstart') {
				image.data('panning', true);
		...
		function onPinch(evt) {
			...
			if (image.data('settings').disabled || image.data('settings').disableZoom) {
				return;
			}
		...
		function onSwipe(evt)
		{
			...
			if (image.data('settings').disabled) {
				return;
			}
		...
		function onMouseWheel(evt)
		{
			...
			if (image.data('settings').disabled || image.data('settings').disableZoom || image.data('panning')) {
				return;
			}
		...
		return this.each(function(index, image)
		{
			if (empty(action)) {
				...
				switch (action) {
					case 'isPanning': return $(image).data('panning'); break;
					case 'isPinching': return $(image).data('pinching'); break;
					case 'isEnabled': return !$(image).data('settings').disabled; break;
					case 'isDisabled': return $(image).data('settings').disabled; break;
					case 'reset': setInitialSizeAndPosition(); break;
				}
			}
			/* TODO
			else if (action == '   ') {
