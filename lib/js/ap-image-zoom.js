...
;(function($) {
	...
	ApImageZoom.prototype = {
		...
		_addWrapper: function() {
			...
			this.$overlay = $('<div></div>').addClass(cssPrefix + 'overlay').appendTo(this.$wrapper);
		...
		_updateCssClasses: function() {
			...
			if (!this.$wrapper.hasClass(cssPrefix + 'mode-' + this.mode)) {
		...
		zoomIn: function(origin) {
			var zoom = this.currentZoom + this.settings.zoomStep;
			this._zoomTo(zoom, origin);
		},
		...
		_setOptions: function(options) {
			for (key in options) {
				...
				if ($.inArray(key, ['cssWrapperClass']) > -1) {
					this._updateCssClasses();
				}
