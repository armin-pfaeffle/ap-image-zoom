 /**
* @license ap-image-zoom.js v2.0.0
* Updated: 17.07.2015
* {DESCRIPTION}
...
;(function($) {
	...
	ApImageZoom.prototype = {
		...
		_obtainNaturalImageSize: function() {
			if (this.mode == 'svg') {
				this.naturalSize = new Size(this.$svgUse.attr('width'), this.$svgUse.attr('height'));
			}
			else {
		...
		_updateSizeAndPosition: function() {
			if (this.mode == 'svg') {
				var x = Math.floor(this.currentPosition.x + this.$image.width() / 2);
				var y = Math.floor(this.currentPosition.y + this.$image.height() / 2);
