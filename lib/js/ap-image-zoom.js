...
;(function($) {
	...
	ApImageZoom.prototype = {
		...
		_init: function() {
			...
			if (this.mode != 'svg' && !this.imageUrl) {
				...
				if (this.mode == 'svg') {
					...
					// var svgClass = this.$target.attr('class');
					// svgClass = (typeof svgClass == 'string' ? svgClass + ' ' : '') + 'apiz-original-svg';
					// this.$target.attr('class', svgClass);
					var mySVG = document.querySelector('#svg');
					mySVG.addClass('apiz-original-svg');
					...
					// use.setAttribute('transform', 'scale(0.5)');
		...
		_obtainNaturalImageSize: function() {
			if (this.mode == 'svg') {
				var clientRect = this.$image.find('use')[0].getBoundingClientRect();
				this.naturalSize = new Size(clientRect.width, clientRect.height);
			}
			else {
		...
		_onMouseWheel: function(evt) {
			...
			if (this.mode == 'svg') {
				console.log('pageX: ' + evt.pageX + '   offset: ' + this.$image.offset().left + '   use.x: ' + this.$image.find('use').attr('x') + '   size: ' + this.currentSize.width);
				var origin = new Scale(
					(evt.pageX - this.$image.offset().left - this.$image.find('use').attr('x')) / this.currentSize.width,
					(evt.pageY - this.$image.offset().top - this.$image.find('use').attr('y')) / this.currentSize.height
				);
				console.log(origin);
			}
			else {
		...
		_updateSizeAndPosition: function() {
			if (this.settings.hardwareAcceleration) {
				if (this.mode == 'svg') {
					use = this.$image.find('use')[0];

					// use.setAttribute('transform', 'scale(' + this.currentZoom + ')');
					var zoom = this.currentZoom;
					var x = (this.currentPosition.x + this.$image.width() / 2) / zoom;
					var y = (this.currentPosition.y + this.$image.height() / 2) / zoom;

					// var clientRect = this.$image.find('use')[0].getBoundingClientRect();
					// var x = (this.$image.width() - clientRect.width) / 2 / zoom;
					// var y = (this.$image.height() - clientRect.height) / 2 / zoom;

					// use.setAttribute('data-x1', this.currentPosition.x)
					// use.setAttribute('data-x2', (this.$image.width() - clientRect.width) / 2)

					// use.setAttribute('transform', 'matrix(' + this.currentZoom + ' 0 0 ' + this.currentZoom + ' ' + this.currentPosition.x + ' ' + this.currentPosition.y + ')');
					use.setAttribute('transform', 'matrix(' + this.currentZoom + ' 0 0 ' + this.currentZoom + ' 0 0)');
					use.setAttribute('x', x);
					use.setAttribute('y', y);
					// use.setAttribute('transform', 'scale(' + this.currentZoom + ') translate(' + (this.currentPosition.x) + ' ' + (this.currentPosition.y) + ')');
					// use.setAttribute('transform', 'translate(' + (this.currentPosition.x) + ' ' + (this.currentPosition.y) + ')');

					// svg.setAttribute('style', 'margin-left: ' + this.currentPosition.x + 'px; margin-top: ' + this.currentPosition.y + 'px');
					// svg.setAttribute('y', this.currentPosition.y)
				}
				else {
